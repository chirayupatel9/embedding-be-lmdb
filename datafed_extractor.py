...
GLOBUS_ENDPOINT_ID = "00af6cee-2134-11f0-aa78-0affeb91e4e5"#os.environ.get("GLOBUS_ENDPOINT_ID")
...
coll_list_resp = df_api.collectionItemsList("c/525610672",count=1)#(os.environ("COLLECTION_NAME"),count = 100000)
...
for idx, each in enumerate(coll_list_resp[0].item):
    ...
    try:
        ig_resp = df_api.dataGet("d/525672244",f"{GLOBUS_ENDPOINT_ID}{GLOBUS_PATH}")
        if dv_resp and dv_resp[0].data and dv_resp[0].data[0].metadata:
            ...
            try:
                ...
                print(f"✅ JSON object saved to file: {res}")
                # else:
                #     print(f"⚠️ File '{res}' already exists. Not overwriting.")
